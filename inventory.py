...
3. When question mentions model type keywords like: 'forecasting', 'hiring', 'fraud', 'collection', 'churn', 'propensity', 'attrition', 'scoring', 'prediction', 'classification', 'retention' etc.
...
4. For COUNT queries   : SELECT COUNT(*) as count FROM df WHERE ...
5. For LIST queries    : SELECT Name_of_the_Model, LOB, Function, Status, Owner, Time_Line FROM df WHERE ...
6. For DESCRIBE queries: SELECT * FROM df WHERE ...
7. For BREAKDOWN       : SELECT <column>, COUNT(*) as model_count FROM df WHERE ... GROUP BY <column> ORDER BY model_count DESC
...
- For counts: state the number clearly with context.DO NOT ask follow-up questions or offer further breakdowns.Just answer what was asked.
...
def run_inventory_query(question: str, accumulated_filters: dict,
    ...
    """
    print("=" * 60)
    print(f"RUN_INVENTORY turn_history: {turn_history}")
    print("=" * 60)
