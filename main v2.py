...
utility_engine      = None            # ← Phase 2 DuckDB utility engine
...
def load_rag():
    ...
    global dashboard_engine, utility_engine
    ...
    # ── Utility Inventory (DuckDB) ─────────────────────────────────────────
    try:
        from rag.utility_inventory import create_utility_engine
        utility_engine = create_utility_engine()
        logger.info("✅ Utility Inventory loaded successfully")
    except Exception as e:
        logger.warning(f"⚠️ Utility Inventory not loaded: {e}")
...
def query(request: QueryRequest):
    try:
        ...
        # ══════════════════════════════════════════════════════════════════
        # UTILITY FLOW — DuckDB Text-to-SQL (Phase 2)
        # ══════════════════════════════════════════════════════════════════
        if intent == "UTILITY":
            if utility_engine is None:
                return format_response(
                    chat_id=chat_id, msg_id=msg_id,
                    question=question, intent="UTILITY",
                    answer=(
                        "The Utility Inventory is not loaded. "
                        "Please check UTILITY_INVENTORY_PATH in your .env file."
                    ),
                    citations=[], is_intent_switch=False, user_id=user_id,
                )

            # Intent switch guard
            if (session["last_intent"] in ["RAG", "INVENTORY", "DASHBOARD"]
                    and (session["chat"] or session["history"])
                    and not confirm_switch
                    and is_followup):
                ...
                return format_response(
                    ...
                    answer=(
                        f"You were in a {source}. "
                        "Switching to utility search will clear your context. "
                        "Do you want to continue?"
                    ),
                    citations=[], is_intent_switch=True, user_id=user_id,
                )

            result = utility_engine.answer_question(rewritten_q)

            if not result["success"]:
                error_msg = result.get("error", "Unknown error")
                logger.error(f"[{chat_id}] Utility query failed: {error_msg}")
                return format_response(
                    chat_id=chat_id, msg_id=msg_id,
                    question=question, intent="UTILITY",
                    answer=f"I encountered an error querying the utility inventory: {error_msg}",
                    citations=[], is_intent_switch=False, user_id=user_id,
                )

            answer_text = result["answer"]

            # Append HTML table if rows returned
            if result.get("result_df") is not None and not result["result_df"].empty:
                df = result["result_df"]
                html_table = df.to_html(
                    index=False, border=1,
                    classes="utility-table", escape=False,
                ).replace('<table', '<table border="1" cellpadding="5" cellspacing="0"')
                answer_text += f"<br><br>{html_table}"

            # Always append SharePoint utility folder link
            answer_text += "<br><br>" + format_link_html(
                "utility", "📁 Open Utility Folder (SharePoint)"
            )

            update_session(
                chat_id=chat_id, filters=session["filters"],
                history=session["history"], intent=intent,
                chat=session["chat"] + [(question, result["answer"])],
                last_subjects=session.get("last_subjects", []),
                last_offer=None, last_category=category,
            )
...
def health():
    return {
        ...
        "utility_ready":       utility_engine is not None,
        ...
        "utility_engine":      "DuckDB (local) + Text-to-SQL",
